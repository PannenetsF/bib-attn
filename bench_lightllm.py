...
def bench(max_length, head_num, head_dim, chunk_size, mean_ratio=0.3, std_ratio=0.3, outlier_ratio=0.01, min_length=10, save_prefix='.'):
    @triton.testing.perf_report(
        triton.testing.Benchmark(
            ...
            line_names=['lightllm', 'bib'],  
            styles=[('blue', '-'), ('green', '-')],  
            ylabel='latency',  
            plot_name='attn score',  
            args={},  
        ))
    def benchmark(batch, provider):
        ...
        return p50, p20, p80
    save = f'{save_prefix}/max-{max_length}-H-{head_num}-h-{head_dim}-C-{chunk_size}-mean-{mean_ratio}-std-{std_ratio}-out-{outlier_ratio}/'
