...
def bib_gqa_qkv_mul_kernel(
    ...
    head_off_2d = head_off[:, None]
    head_off__2d = head_off[None, :]
    chunk_off = tl.arange(0, CHUNK_SIZE)
    q_head_num_off = tl.arange(0, Q_HEAD_NUM)
    q_head_mask = q_head_num_off < GROUP_SIZE
    q_head_mask_2d = q_head_mask[:, None]
    q_head_idx = kv_head_idx * GROUP_SIZE + q_head_num_off
    q_head_idx_2d = q_head_idx[:, None]
    q_off = cur_batch * q_bs + q_head_idx_2d * q_H + head_off__2d * q_h  # [H, h]
    ...
    k_off = (k_len[None, :] + cur_start) * k_D + kv_head_idx * k_H + head_off_2d * k_h  # [chunk_size, head_dim]
    v_off = (k_len[:, None] + cur_start) * v_D + kv_head_idx * v_H + head_off__2d * v_h
    k_mask = k_len < cur_length
    k_mask_2d = k_mask[:, None]
    k_mask__2d = k_mask[None, :]
    k_mat = tl.load(k_tensor + k_off)
    score = tl.dot(q_vec, k_mat)  # [H, chunk_size]
    score *= sm_scale
    score = tl.where(q_head_mask_2d & k_mask__2d, score, float('-inf'))
    s_max = tl.max(score, axis=1)
    v_mat = tl.load(v_tensor + v_off)
    ...
    tl.store(nume_tensor + block_idx * nume_bs + q_head_idx_2d * nume_H + head_off__2d * nume_h, s_exp_v,
             mask=q_head_mask_2d)
...
def bib_gqa_decoding(
    ...
    grid = lambda META: (block_num, k_head_num)
...
if __name__ == '__main__':
    ...
    kv_tensor = torch.randn(2, *k_shape, dtype=torch.float32).cuda()
    k_tensor = kv_tensor[0]
    v_tensor = kv_tensor[1]
