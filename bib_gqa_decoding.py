...
@triton.jit
def bib_gqa_qkv_mul_q_first_kernel(
    ...
    q_off = cur_batch * q_bs + q_head_idx * q_H + head_off * q_h
    q_vec = tl.load(q_tensor + q_off)  # [head_dim]

    k_len = cur_chunk * CHUNK_SIZE + chunk_off
    k_off = (k_len[:, None] + cur_start) * k_D + kv_head_idx * k_H + head_off[None, :] * k_h
    ...
    # shape (1, ), (1, ), (head_dim, )
    tl.store(deno_tensor + block_idx * deno_bs + q_head_idx * deno_H + 0 * deno_dim, s_max)
    tl.store(deno_tensor + block_idx * deno_bs + q_head_idx * deno_H + 1 * deno_dim, s_exp_sum)
    tl.store(nume_tensor + block_idx * nume_bs + q_head_idx * nume_H + head_off * nume_h, s_exp_v_sum)


@triton.jit
def bib_gqa_qkv_mul_kv_first_tc_kernel(
        q_tensor, q_bs, q_H, q_h,
        k_tensor, k_D, k_H, k_h,
        v_tensor, v_D, v_H, v_h,
        deno_tensor, deno_bs, deno_H, deno_dim,  # 2dim, max and sum
    ...
    k_mask = k_len < cur_length
    k_mat = tl.load(k_tensor + k_off, mask=k_mask[:, None], other=0.0)
    k_mat = tl.trans(k_mat)
    score = tl.dot(q_vec, k_mat)  # [H, chunk_size]
    score = tl.where(q_head_mask[:, None] & k_mask[None, :], score * sm_scale, -1e9)
...
@triton.jit
def bib_gqa_qkv_mul_kv_first_tc_notrans_kernel(
        q_tensor, q_bs, q_H, q_h,
        k_tensor, k_D, k_H, k_h,
        v_tensor, v_D, v_H, v_h,
        deno_tensor, deno_bs, deno_H, deno_dim,  # 2dim, max and sum
...
@triton.jit
def bib_gqa_qkv_mul_q_first_tc_kernel(
        q_tensor, q_bs, q_H, q_h,
        k_tensor, k_D, k_H, k_h,
        v_tensor, v_D, v_H, v_h,
        deno_tensor, deno_bs, deno_H, deno_dim,  # 2dim, max and sum
    ...
    q_head_off = tl.arange(0, 16)
    q_head_mask = q_head_off < 1
    q_off = cur_batch * q_bs + q_head_off[:, None] * q_H + head_off[None, :] * q_h
    q_vec = tl.load(q_tensor + q_off, mask=q_head_mask[:, None], other=0.)  # [head_dim]
    ...
    k_mask = k_len < cur_length
    k_mat = tl.load(k_tensor + k_off, mask=k_mask[:, None], other=0.0)
    # q_vec [1, head_dim], k_mat [chunk_size, head_dim]
    k_mat = tl.trans(k_mat)
    score = tl.dot(q_vec, k_mat)
    score = tl.where(q_head_mask[:, None] & k_mask[None, :], score * sm_scale, -1e9)  # [H, chunk_size]

    v_mat = tl.load(v_tensor + v_off, mask=k_mask[:, None], other=0.0)  # [chunk_size, head_dim]
    s_max = tl.max(score, axis=1)
    s_exp = tl.exp(score - s_max[:, None])
    s_exp_sum = tl.sum(s_exp, axis=1)  # [H]

    s_exp_v = tl.dot(s_exp, v_mat)  # [H, head_dim]
    # s_exp_v_sum = tl.sum(s_exp_v, axis=1) # [H]

    # save s_max, s_exp_sum, s_exp_v
    # shape (1, ), (1, ), (head_dim, )
    tl.store(deno_tensor + block_idx * deno_bs + q_head_off * deno_H + 0 * deno_dim, s_max, mask=q_head_mask)
    tl.store(deno_tensor + block_idx * deno_bs + q_head_off * deno_H + 1 * deno_dim, s_exp_sum,
             mask=q_head_mask)
    tl.store(nume_tensor + block_idx * nume_bs + q_head_off[:, None] * nume_H + head_off[None, :] * nume_h, s_exp_v,
             mask=q_head_mask[:, None])


@triton.jit
def bib_gqa_qkv_mul_q_first_tc_notrans_kernel(
        q_tensor, q_bs, q_H, q_h,
        k_tensor, k_D, k_H, k_h,
        v_tensor, v_D, v_H, v_h,
        deno_tensor, deno_bs, deno_H, deno_dim,  # 2dim, max and sum
...
class BiBConfig:
    qFirstTensorCore = 'qFirstTensorCore'
    qFirstMMA = 'qFirstMMA'
    kvFirstTensorCore = 'kvFirstTensorCore'
    kvFirstTensorCoreNoTrans = 'kvFirstTensorCoreNoTrans'
    qFirstTensorCoreNoTrans = 'qFirstTensorCoreNoTrans'

    def all_config(self):
        return [
            self.qFirstTensorCoreNoTrans,
            self.qFirstTensorCore,
            self.qFirstMMA,
            self.kvFirstTensorCoreNoTrans,
            self.kvFirstTensorCore
        ]

    @staticmethod
    def config_to_kernel(cfg):
        if cfg == BiBConfig.qFirstMMA:
            return bib_gqa_qkv_mul_q_first_kernel
        elif cfg == BiBConfig.qFirstTensorCore:
            return bib_gqa_qkv_mul_q_first_tc_kernel
        elif cfg == BiBConfig.kvFirstTensorCore:
            return bib_gqa_qkv_mul_kv_first_tc_kernel
        elif cfg == BiBConfig.kvFirstTensorCoreNoTrans:
            return bib_gqa_qkv_mul_kv_first_tc_notrans_kernel
        elif cfg == BiBConfig.qFirstTensorCoreNoTrans:
            return bib_gqa_qkv_mul_q_first_tc_notrans_kernel
        else:
...
def bib_gqa_decoding(
    ...
    if config is None:
        config = BiBConfig.qFirstMMA
    kernel = BiBConfig.config_to_kernel(config)
