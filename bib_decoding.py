...
def bib_gqa_ref_request_max_reduce(
    ...
    head_off = tl.arange(0, HEAD_DIM)
    block_idx = tl.load(request_to_block + request_id * r2b_bs + r2b_blk * chunk_off, mask=chunk_off < CHUNK_NUM, other=-1)
    block_mask = block_idx != -1
    bmask = tl.sum(block_mask)

    s_max = tl.load(deno_tensor + block_idx * deno_bs + head_idx * deno_H + 0 * deno_dim, mask=block_mask, other=-1e9) # (CN, )
    s_exp_sum = tl.load(deno_tensor + block_idx * deno_bs + head_idx * deno_H + 1 * deno_dim, mask=block_mask, other=0) # (CN, )
    s_exp_v = tl.load(nume_tensor + block_idx[:, None] * nume_bs + head_idx * nume_H + head_off[None, :] * nume_h, mask=block_mask[:, None], other=0.) # (CN, h)
