...
@triton.jit
def _fwd_kernel_flash_decode_stage1(block_batch_ids, block_start_indexes, Q, K, V, sm_scale, Req_to_tokens, B_req_idx,
                                    B_Seqlen, Mid_O,  # [batch, head, seq_block_num, head_dim]
                                    Mid_O_LogExpSum,  # [batch, head, seq_block_num]
                                    stride_req_to_tokens_b, stride_req_to_tokens_s, stride_qbs, stride_qh, stride_qd,
                                    stride_kbs, stride_kh,
                                    stride_kd, stride_vbs, stride_vh, stride_vd, stride_mid_ob, stride_mid_oh,
                                    stride_mid_os, stride_mid_od,
                                    stride_mid_o_eb, stride_mid_o_eh, stride_mid_o_es, gqa_group_size,
                                    Q_HEAD_NUM: tl.constexpr,
                                    BLOCK_SEQ: tl.constexpr, BLOCK_DMODEL: tl.constexpr, BLOCK_N: tl.constexpr):
    ...
    for _ in range(0, need_store, 1):
        ...
        off_mid_o = cur_batch * stride_mid_ob + cur_q_head_range[:,
                                                None] * stride_mid_oh + seq_block_index * stride_mid_os + offs_d[None,
                                                                                                          :]
...
@torch.no_grad()
def flash_decode_stage1(block_batch_ids, block_start_indexes, q, k, v, Req_to_tokens, B_req_idx, B_seq_len, mid_out,
                        mid_out_logsumexp, block_seq):
    BLOCK_SEQ = block_seq
    BLOCK_N = 16
    ...
    _fwd_kernel_flash_decode_stage1[grid](block_batch_ids, block_start_indexes, q, k, v, sm_scale, Req_to_tokens,
                                          B_req_idx, B_seq_len, mid_out, mid_out_logsumexp, Req_to_tokens.stride(0),
                                          Req_to_tokens.stride(1), q.stride(0),
                                          ...
                                          mid_out_logsumexp.stride(1), mid_out_logsumexp.stride(2), gqa_group_size,
                                          Q_HEAD_NUM=max(16, triton.next_power_of_2(gqa_group_size)),
                                          BLOCK_SEQ=BLOCK_SEQ, BLOCK_DMODEL=Lk,
                                          BLOCK_N=BLOCK_N, num_warps=2, num_stages=2, )
    return


@triton.jit
def _fwd_kernel_flash_decode_stage2(B_Seqlen, Mid_O,  # [batch, head, seq_block_num, head_dim]
                                    Mid_O_LogExpSum,  # [batch, head, seq_block_num]
                                    O,  # [batch, head, head_dim]
                                    stride_mid_ob, stride_mid_oh, stride_mid_os, stride_mid_od, stride_mid_o_eb,
                                    stride_mid_o_eh, stride_mid_o_es,
                                    stride_obs, stride_oh, stride_od, BLOCK_SEQ: tl.constexpr,
...
def flash_decode_stage2(mid_out, mid_out_logexpsum, B_Seqlen, O, block_seq):
    ...
    _fwd_kernel_flash_decode_stage2[grid](B_Seqlen, mid_out, mid_out_logexpsum, O, mid_out.stride(0), mid_out.stride(1),
                                          mid_out.stride(2), mid_out.stride(3), mid_out_logexpsum.stride(0),
                                          mid_out_logexpsum.stride(1),
                                          mid_out_logexpsum.stride(2), O.stride(0), O.stride(1), O.stride(2),
                                          BLOCK_SEQ=block_seq, BLOCK_DMODEL=Lk,
...
def gqa_token_decode_attention_flash_decoding(q, cache_k, cache_v, out,
    ...
    block_batch_ids = torch.from_numpy(np.concatenate(
        [np.full(((b_seq_len_numpy[batch_id] + BLOCK_SEQ - 1) // BLOCK_SEQ,), fill_value=batch_id, dtype=np.int32) for
         batch_id in range(len(b_seq_len_numpy))], axis=0)).cuda()

    block_start_indexes = torch.from_numpy(
        np.concatenate([np.arange(0, seq_len, BLOCK_SEQ, dtype=np.int32) for seq_len in b_seq_len_numpy],
                       axis=0)).cuda()
    ...
    flash_decode_stage1(block_batch_ids, block_start_indexes, q.view(calcu_shape1), cache_k,
                        ...
                        b_seq_len, mid_o, mid_o_logexpsum, BLOCK_SEQ)
    flash_decode_stage2(mid_o, mid_o_logexpsum, b_seq_len, o_tensor.view(calcu_shape1), BLOCK_SEQ)
    return o_tensor
