...
def bib_gqa_ref_qk_mul_kernel(
    ...
    for chunk_idx in tl.static_range(CHUNK_SIZE):
        k_len = cur_chunk * CHUNK_SIZE + chunk_idx
        k_idx = cur_start + k_len
        k_off = k_idx * k_D + head_idx * k_H + head_off * k_h
        k_mask = k_len < cur_length
        k_vec = tl.load(k_tensor + k_off, mask=k_mask, other=0.0)
...
def bib_gqa_ref_qk_mul(
    ...
    bib_gqa_ref_qk_mul_kernel[grid](
...
@triton.jit
def bib_gqa_ref_qk_mul_kernel_remove_k_load(
    q_tensor, q_bs, q_H, q_h,
    k_tensor, k_D, k_H, k_h,
    score_tensor, s_bs, s_H, s_L,
    block_to_request, b2r_bs,
    ...
    for chunk_idx in tl.static_range(CHUNK_SIZE):
        ...
        k_off = k_idx * k_D + head_idx * k_H + head_off * k_h
        k_mask = k_len < cur_length
        k_vec = tl.zeros(shape=(HEAD_DIM,), dtype=tl.float32)
...
@torch.no_grad()
def bib_gqa_ref_qk_mul_remove_k_load(
    q_tensor,
    k_tensor,
    score_tensor,
    block_to_request,
    ...
    bib_gqa_ref_qk_mul_kernel_remove_k_load[grid](
        q_tensor, q_tensor.stride(0), q_tensor.stride(1), q_tensor.stride(2),
        k_tensor, k_tensor.stride(0), k_tensor.stride(1), k_tensor.stride(2),
        score_tensor, score_tensor.stride(0), score_tensor.stride(1), score_tensor.stride(2),
        block_to_request, block_to_request.stride(0),
...
@triton.jit
def bib_gqa_ref_qk_mul_kernel_remove_block(
    q_tensor, q_bs, q_H, q_h,
    k_tensor, k_D, k_H, k_h,
    score_tensor, s_bs, s_H, s_L,
    block_to_request, b2r_bs,
...
@torch.no_grad()
def bib_gqa_ref_qk_mul_remove_block(
    q_tensor,
    k_tensor,
    score_tensor,
    block_to_request,
    ...
    bib_gqa_ref_qk_mul_kernel_remove_block[grid](
        q_tensor, q_tensor.stride(0), q_tensor.stride(1), q_tensor.stride(2),
        k_tensor, k_tensor.stride(0), k_tensor.stride(1), k_tensor.stride(2),
        score_tensor, score_tensor.stride(0), score_tensor.stride(1), score_tensor.stride(2),
        block_to_request, block_to_request.stride(0),
